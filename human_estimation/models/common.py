        ...
        # 예측 결과가 있으면
            if pred is not None:
                ...
                for c in pred[:, -1].unique():
                    n = (pred[:, -1] == c).sum()  # 클래스별 검출 수
                    str += f"{n} {self.names[int(c)]}{'s' * (n > 1)}, "  # 문자열에 추가

                # show, save 또는 render가 True인 경우
                if show or save or render:
                    for *box, conf, cls in pred:  # xyxy, confidence, class
                        label = f'{self.names[int(cls)]} {conf:.2f}'
                        plot_one_box(box, img, label=label, color=colors[int(cls) % 10])

            # 이미지를 Image 객체로 변환 (numpy 배열에서 Image 객체로 변환)
            img = Image.fromarray(img.astype(np.uint8)) if isinstance(img, np.ndarray) else img

            # pprint가 True인 경우 문자열 출력
            if pprint:
                print(str.rstrip(', '))

            # show가 True인 경우 이미지 표시
            if show:
                img.show(self.files[i])  # 이미지 표시

            # save가 True인 경우 이미지 저장
            if save:
                f = self.files[i]
                img.save(Path(save_dir) / f)  # 이미지 저장
                print(f"{'Saved' * (i == 0)} {f}", end=',' if i < self.n - 1 else f' to {save_dir}\n')

            # render가 True인 경우 이미지를 numpy 배열로 변환하여 저장
            if render:
